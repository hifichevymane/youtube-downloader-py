...
def main():
    ...
    download_option = int(input(
        'What do you wanna download? 1 - only video, 2 - audio from video, 3 - video and audio: '))
    ...
    if 'playlist?' in video_url:
        utils.download_playlist(
            video_url, DOWNLOADS_FOLDER, download_option=download_option)
    ...
    if download_option == 2:
        utils.convert_mp4_to_mp3(
            path_to_video=path[0], name_of_file=path[1], delete_video=True)
